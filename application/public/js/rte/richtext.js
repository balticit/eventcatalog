...
function writeRichText(rte, html, width, height, buttons, readOnly) {
	...
	//		document.writeln('		<td><img class="rteImage" src="' + imagesPath + 'redo.gif" width="25" height="24" alt="Redo" title="Redo" onClick="rteCommand(\'' + rte + '\', \'redo\')"></td>');
		...
		if (!readOnly) document.writeln('<br /><input type="checkbox" id="chkSrc' + rte + '" onclick="toggleHTMLSrc(\'' + rte + '\',' + buttons + ');" />&nbsp;<label for="chkSrc' + rte + '">Режим HTML</label>');
		...
		*/
		document.writeln('<table class="rteBack" cellpadding="0" cellspacing="0" id="Buttons2_' + rte + '" width="' + tablewidth + '">');
		document.writeln('	<tr>');
		document.writeln('		<td>Управление&nbsp;изображениями: </td>');
		document.writeln('		<td><img class="rteImage" src="' + imagesPath + 'left_just.gif" width="25" height="24" alt="Align Left" title="Align Left" onClick="rteImExt_imageCommand(\'' + rte + '\', \'alignleft\', \'\')"></td>');
		document.writeln('		<td><img class="rteImage" src="' + imagesPath + 'centre.gif" width="25" height="24" alt="Center" title="Center" onClick="rteImExt_imageCommand(\'' + rte + '\', \'aligncenter\', \'\')"></td>');
		document.writeln('		<td><img class="rteImage" src="' + imagesPath + 'right_just.gif" width="25" height="24" alt="Align Right" title="Align Right" onClick="rteImExt_imageCommand(\'' + rte + '\', \'alignright\', \'\')"></td>');
		document.writeln('		<td><img class="rteImage" src="' + imagesPath + 'justifyfull.gif" width="25" height="24" alt="Justify Full" title="Justify Full" onclick="rteImExt_imageCommand(\'' + rte + '\', \'alignnone\', \'\')"></td>');
		document.writeln('		<td width="100%"></td>');
		document.writeln('	</tr>');
		document.writeln('</table>');
		//document.writeln('<hr noshade="noshade" size="1"><a href="JavaScript: rteImExt_addImage(\''+rte+'\');">добавить рисунок</a>');
		document.writeln('<table id="'+rte+'_imagesTable">');
...
function enableDesignMode(rte, html, readOnly) {
	...
	if (document.all) {
		...
		if (!readOnly) {
			...
			* RTE Image Extension
			*/
			frames[rte].document.attachEvent("oncontrolselect", function evt_ie_click(event) {rteImExt_ieClick(event, rte);});
		}
	} else {
		try {
			...
			try {
				...
				if (isGecko && !readOnly) {
					...
					* RTE Image Extension
					*/
					oRTE.addEventListener("click", new Function("evt", "rteImExt_geckoClick(evt, '" + rte + "');"), true);
				}
			} catch (e) {
...
var g_rteImExt_activeImages = new Array();

var g_rteImExt_prevImagePath = new Array();

var g_rteImExt_uids = 0;

function rteImExt_imageCommand(rte, command)
{
	if(!g_rteImExt_activeImages[rte])
	{
		return;
	}
	...
	{
		g_rteImExt_activeImages[rte].align = "left";
	}
	else if(command == "alignright")
	{
		g_rteImExt_activeImages[rte].align = "right";
	}
	else if(command == "aligncenter")
	{
		g_rteImExt_activeImages[rte].align = "center";
	}
	else if(command == "alignnone")
	{
		g_rteImExt_activeImages[rte].align = "";
	}
}

function rteImExt_ieClick(evt, rte)
{
	if(evt.srcElement.tagName == "IMG")
	{
		g_rteImExt_activeImages[rte] = evt.srcElement;
	}
}

function rteImExt_geckoClick(evt, rte)
{
	if(evt.target.tagName == "IMG")
	{
		g_rteImExt_activeImages[rte] = evt.target;
	}
}

/**
* Обработчик события onchange у `input type=file`,
* в которых юзер выбирает картинки.
* Вся эта бадяга нужна для того, что бы отслеживать, когда
* юзер выбрал файл. После этого если url файла поменялся,
* в окне редактирования тоже сменятся все урли
*
* @return void
* @access public
*/
function rteImExt_OnChangeInputFile(fileElement, rte)
{
	/**
	* Получаем элемент `img`, который отображается рядом
	* с полем для выбора файла
	*/
	var pathHiddenInput = document.getElementById(fileElement.id + "_imagePath");

	/**
	* Собственно новый url картинки
	*/
	imageUrl = "file:///" + fileElement.value.replace(/\\/g, "/");
	if(!imageUrl.match(/^[a-z_ 0-9\/\\\\.\:]+$/i))
	{
		alert("Имя картинки должно содержать только латинские буквы,\nсимвол подчеркивания и цифры. Выберите другое изображение");
		return false;
	}
	

	var previewImg = document.getElementById(fileElement.id + "_imagePreviewHolder").childNodes.item(0);
	...
	var imagesCollection = oRTE.document.getElementsByTagName("IMG");
	for(var i = 0; i < imagesCollection.length; i++)
	{
		var image = imagesCollection[i];

		if(image.src == g_rteImExt_prevImagePath[fileElement.id])
		{
			image.src = imageUrl;
		}
	}
	g_rteImExt_prevImagePath[fileElement.id] = imageUrl;
	pathHiddenInput.value = imageUrl;
}

function rteImExt_getRteControl(rte)
{
	var oRTE;
	if (document.all) {
		oRTE = frames[rte];
...
function rteImExt_addImage(rte)
{
	++g_rteImExt_uids;
	...
	var table = document.getElementById(rte + "_imagesTable");

	var row = table.insertRow(-1);
	var cellInput = row.insertCell(-1);
	cellInput.innerHTML =
		'<input type="file" ' +
			'id="rte_is_' + g_rteImExt_uids + '" ' +
			'name="' + rte + '_images[' + g_rteImExt_uids + ']" ' +
			'onchange="rteImExt_OnChangeInputFile(this, \'' + rte + '\');"/><br/>' +
		'<a href="JavaScript: rteImExt_delImage(\'' + rte + '\', \'rte_is_' + g_rteImExt_uids + '\');">удалить</a>';

	cellInput.innerHTML +=
		'<input type="hidden" '+
			'id="rte_is_'+g_rteImExt_uids+'_imagePath" '+
			'name="' + rte + '_paths[' + g_rteImExt_uids + ']"/>';

	/**
	* Ячейка с картинкой
	*/
	var cellImage = row.insertCell(-1);
	cellImage.id = "rte_is_" + g_rteImExt_uids + "_imagePreviewHolder";
	cellImage.innerHTML = '<img/>';
}

function rteImExt_delImage(rte, fileElementId)
{
	var oRTE;
	if (document.all) {
		oRTE = frames[rte];
	...
	var imageUrl = "file:///" + fileElement.value.replace(/\\/g, "/");

	var imagesCollection = oRTE.document.getElementsByTagName("IMG");
	for(var i = 0; i < imagesCollection.length;)
	{
		var imageElement = imagesCollection[i];
		...
		{
			// Инкремент делается только тогда, когда не было удаления картинки
			// Ведь при её удалении, размер коллекции изменяется и i++ проскочит
			// следующий за удаленным элемент
			i++;
		}
	}

	// Удаление строки таблицы
	fileElement.parentNode.parentNode.parentNode.removeChild(fileElement.parentNode.parentNode);
}