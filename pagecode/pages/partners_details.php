<?php
	class partners_details_php extends CPageCodeHandler 
	{
		public function partners_details_php()
		{
      ...
      $av_rwParams = array("id");
		  CURLHandler::CheckRewriteParams($av_rwParams);  
			...
			$unit = SQLProvider::ExecuteQuery("SELECT * from tbl__partners where tbl_obj_id = $id");
			if (sizeof($unit)==0)
			{
			...
			$photos = SQLProvider::ExecuteQuery("SELECT
						  p.*
						FROM 
						  `tbl__partners2photo`  ap
						  inner join `tbl__photo` p 
						  on ap.photo_id = p.tbl_obj_id 
						  where partners_id=$id limit 8");
			$unit[0]["photo_list_1"] = "";
			$unit[0]["photo_list_2"] = "";
