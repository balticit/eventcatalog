...
class agency_php extends CPageCodeHandler
{
    ...
    SQLProvider::ExecuteNonReturnQuery("update $tbl set last_visit_date=NOW() WHERE tbl_obj_id = $user->id AND last_visit_date<DATE_SUB(NOW(), INTERVAL 1 MINUTE) ");
        ...
        $metadata->keywords = "event агентства, ивент агентства, агентства деловых меропри€тий, концертные агентства, букинговые агентства, выставочные агентства, свадебные агентства, событийный туризм, инсентив";
        $metadata->description = "EventCatalog.ru Ц каталог event агентств, организаторов меропри€тий, концертных и букинговых агентствЕ и многое другое!";
        ...
        where t.tbl_obj_id=$activity
          and r.active=1");
            ...
            //выставл€ем keywords, description и текст дл€ каждого раздела
        ...
        from tbl__agency_type where tbl_obj_id = $activity");
            if (sizeof($info) > 0) {
                ...
                if (!empty($info["page_title"]))
                    ...
                    $this->GetControl('title')->text = $info["title"] . " -  аталог агентств - ";
        ...
        order by if(tbl_obj_id=$first,0,1), pro_type desc, pro_cost desc, pro_date_pay desc, title limit $sp,$this->pagesize");
            ...
            foreach ($agencies as &$agency)
            {
                ...
                /*$photos = SQLProvider::ExecuteQuery("select 1 from tbl__agency_photos where parent_id = ".$agency['tbl_obj_id']);
                    if (sizeof($photos)) {
                        $agency['links'] .= '<a class="common" href="/agency/'.$agency['title_url'].'?page=photos" style="margin: 0 10px 0 0">‘ото</a>';
                    }
                    if ($agency["youtube_video"]) {
                        $agency['links'] .= '<a class="common" href="/agency/'.$agency['title_url'].'?page=video" style="margin: 0 10px 0 0">¬идео</a>';
                    }*/
            ...
            $this->GetControl('title')->text = " аталог агентств - ";
            /*recommended list*/
            ...
            */
                ...
                $res_news[$key]["logo"] = $res_news[$key]["logo_image"];
        			...
        			case 'area': $res_news[$key]['sub'] = 'Ќовость площадки'; break;
        			case 'artist': $res_news[$key]['sub'] = 'Ќовость артиста'; break;
        			case 'contractor': $res_news[$key]['sub'] = 'Ќовость подр€дчика'; break;
        			case 'agency': $res_news[$key]['sub'] = 'јгентство'; break;
        			}
        		}
