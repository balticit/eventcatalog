			...
			buttons: {
				...
				'Нет': function() {
                  ...
                  /*
										...
										return false;});
										\$('.fav_del_cross').click( function() {
											if(confirm('Вы уверены что хотите удалить '+\$(this).parents('tr').find('a').html()+' из оценок?')) {
												\$(this).next().attr('checked','checked');
												\$('#fav_del_form').submit();
											}
										});
      ...
      case "my_messages":
          ...
          echo '<pre>';
			...
			/*	"my_messages/action/blacklist" => array(
    			...
    			if ($new_count>0)
    			{
    			  $u_cab_menu->dataSource["/r_cabinet/data/my_messages"]["title"] = "Мои сообщения&nbsp;($new_count)";
    				$left_menu["my_messages/action/inbox"]["text"] .= "&nbsp;($new_count)";
    			}
          else { 
            $u_cab_menu->dataSource["/r_cabinet/data/my_messages"]["title"] = "Мои сообщения&nbsp;";
				...
				else
				{
					...
					else
					{
						...
						if ($m_action=="reply"||$m_action=="compose")
						{
							...
							$cab["main_area"].='<div class="message grey_message"><span style="float:right">'.$reply_mess["time"].'</span> <span style="margin-right:140px">'.ProcessMessage($reply_mess["text"])."</span></div>";
...
$left_menu = array(
      ...
      <script type=\"text/javascript\">
					var del_selected = false;
					\$('#btn_fav_del').click(function(){
						if(confirm('Вы уверены что хотите удалить все из новостей?')) {
							\$('.fav_del').attr('checked','checked');
							\$('#fav_del_form').submit();
						}
					return false;});
					\$('.fav_del_cross').click( function() {
						if(confirm('Вы уверены что хотите удалить '+\$(this).parents('tr').find('a').html()+' из новостей?')) {
							\$(this).next().attr('checked','checked');
							\$('#fav_del_form').submit();
						}
					});
