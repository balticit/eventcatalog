...
class contractor_php extends CPageCodeHandler
{
    ...
    SQLProvider::ExecuteNonReturnQuery("update $tbl set last_visit_date=NOW() WHERE tbl_obj_id = $user->id AND last_visit_date<DATE_SUB(NOW(), INTERVAL 1 MINUTE) ");
        ...
        $metadata->keywords = "аренда оборудования для мероприятийи праздников, техническое обеспечение мероприятий: аренда звука, аренда свет, кейтеринг, аренда мебели, декорирование мероприятий, аренда аттракционов";
        $metadata->description = "EventCatalog.ru - полный список прокатчиков оборудования для организаторов мероприятий: аренда звука, аренда света, декорирование мероприятий, кейтеринг… и многое другое!";
