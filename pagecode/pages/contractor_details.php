...
class contractor_details_php extends CPageCodeHandler
{
    ...
    public function PreRender()
    {
        ...
        if ($show_ilike) {
            ...
            $unit["btn_i_like"]='<img class="btn_ilike" src="/images/rating/hand_contractor.png">&nbsp;&nbsp;</td><td><form method="post"><input type="hidden" name="action" value="unlike"><a href="" class="black" onclick="javascript: $(this).parent().submit(); return false;">Больше не нравится</a></form>';
        }
		...
		if ($fav_add) {
			...
			$unit["fav_link"] = '<a class="contractor" href="/contractor/'.$id_str.'?add=favorite" '.$msg.'>Добавить в избранное</a>';
		}
		else {
			$msg = "";
			if (!$user->authorized) $msg = 'onclick="javascript: ShowFavMessage(); return false;"';
			$unit["fav_link"] = '&nbsp;&nbsp;&nbsp;<a class="contractor" href="/contractor/'.$id_str.'?delete=favorite" '.$msg.'>Убрать из избранного</a>';
		}
        //process activities
