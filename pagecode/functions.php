...
function onSiteTime($regdate = ''){
    ...
    return ($years>0?declension($years, ' год', ' года', ' лет').' ':'').($months>0?declension($months, ' месяц', ' месяца', ' месяцев'):'').' '.($months>0||$years>0?'':($days>0?declension($days,' день',' дня',' дней'):'1 день'));
}
...
if($months > 0 || $years > 0) { return "<span class=\"lastVisitSite\">Был на сайте:</span> больше месяца назад";}
...
//if($years > 0) { return "<span class=\"lastVisitSite\" >Был на сайте:</span> больше года назад"; }
if($months > 0 || $years > 0) { return "больше месяца назад";}
