...
function generateHeader($username = "user") {
    ...
    return "<tr>
		...
		<tr>
		<td style='padding-top:10px;color:black;padding-left:10px' colspan='2'>������������, $username!<br/>
		�� ������ � $start_date $strmonthstart �� $day_end $strmonthend $year ���� �� ������� eventcatalog.ru ����������:
		</td>
		</tr>";
}

function generateFooter() {
    return "<tr><td style='padding-top:5px;color:black;text-decoration:none;padding-left:10px' colspan='2'>������� ��� event-������ � �� ������� �� ��������� �����!<br/><br/>
    � ���������,<br/>
    ��� ���������� ��������<br/>
    ...
    <p style='color:gray;font-size:8pt;'>
        ���� �� �� ������� �������� �������� �� ������� ��� ������������� �����������, ��������� �� <a style='color:gray;' href='http://eventcatalog.ru/u_cabinet'>���� ������</a>.
    </p
    </td>
    </tr>";
}
    ...
    return "<tr style='height: 81px;'>
		<td style='padding: 0;margin: 0;cursor: pointer;padding-left:10px;padding-top:5px;'>
			...
			<td style='padding-left: 10px;padding-top:5px;' valign='top'>
				...
				<span style='color:black'>�����: $city_name</span><br>
                            <div style='position:relative; width: 700px; overflow:hidden; height:50px; z-index:0; margin-right: 5px; color:black;'>
                                $text<br/>
                                <a href='$base/$type/$title_url' style='color: blue;text-decoration:none;'>������� �� �������� ��������</a>
                            </div>
			</td>
            </tr>";
}
...
function prepareBody() {	
    ...
    if (count($publics) != 0) {
        $html .="<tr><td colspan='2' style='color:black;padding-top:10px;font-weight:bold;padding-left:10px;'>����� ����� �� EVENT TV:</td></tr>";
    ...
    if (count($eteka) != 0) {
        $html.="<tr><td colspan='2' style='color:black;padding-top:10px;font-weight:bold;padding-left:10px;'>����� ������ � ����������:</td></tr>";
    ...
    if (count($res_news) != 0) {
        ...
        foreach ($res_news as $key => $val) {
            ...
            switch ($val['resident_type']) {
                case 'area': $res_news[$key]['sub'] = '������� ��������';
                    $res_news[$key]['color'] = "#39F";
                    break;
                case 'artist': $res_news[$key]['sub'] = '������� �������';
                    $res_news[$key]['color'] = "#F06";
                    break;
                case 'contractor': $res_news[$key]['sub'] = '������� ����������';
                    $res_news[$key]['color'] = "#F05620";
                    break;
                case 'agency': $res_news[$key]['sub'] = '������� ���������';
                    $res_news[$key]['color'] = "#9C0";
                    break;
            }
        }
        foreach ($res_news as $doc) {
            $html.=renderResidentNews($doc, "������ ������� ���������");
        }
    }
    //new_resident
    ...
    if (count($newArea) != 0 || count($newArtist) != 0 || count($newAgency) != 0 || count($newContractor) != 0) {
        ...
        foreach ($newArea as $doc) {
            ...
            $html .= renderNewResident($doc, "#39F", "��������", "area");
        }
        foreach ($newArtist as $doc) {
            ...
            $doc['description'] = strip_tags(substr($doc['description'], 0, 150) . "...");
            $html .= renderNewResident($doc, "#F06", "������", "artist");
        }
        foreach ($newAgency as $doc) {
            ...
            $doc['description'] = strip_tags(substr($doc['description'], 0, 150) . "...");
            $html .= renderNewResident($doc, "#9C0", "��������", "agency");
        }
        foreach ($newContractor as $doc) {
            ...
            $doc['description'] = strip_tags(substr($doc['description'], 0, 150) . "...");
            $html .= renderNewResident($doc, "#F05620", "���������", "contractor");
        }
    }
    mysql_close($dbLink);
