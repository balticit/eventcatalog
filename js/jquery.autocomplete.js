...
(function($) {
  ...
  Autocomplete.prototype = {
    ...
    initialize: function() {
      ...
      $('<div id="' + this.mainContainerId + '" style="position:absolute;"><div class="autocomplete-w1"><div class="autocomplete-w2"><div class="autocomplete" id="' + autocompleteElId + '" style="display:none; width:' + this.options.width + 'px;"></div></div></div></div>').appendTo('body');
...
//               console.log(msg);  
    ...
    suggest: function() {
      ...
      if (this.options.highlight) {
        var matches = this.currentValue.match(/\w+/g);
        if (matches == null) {
           var re = new RegExp('$' + this.currentValue, 'gi');
        }
        else {
           var re = new RegExp('\\b' + matches.join('|\\b'), 'gi');
        }
      }
      var me = this;
      ...
      this.container.html('');
      for (var i = 0; i < len; i++) {
        div = $((me.selectedIndex === i ? '<div class="selected"' : '<div') + ' title="' + me.suggestions[i] + '">' + Autocomplete.highlight(me.suggestions[i], re) + '</div>');
